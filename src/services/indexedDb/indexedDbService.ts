...
async function forceDeleteDatabase(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      console.warn('[IndexedDB] Force delete timeout, resolving anyway')
      dbInstance = null
      dbInitPromise = null
      resolve()
    }, 5000)
    ...
    request.onblocked = () => {
      ...
      // Don't wait forever, resolve after a short delay
      setTimeout(() => {
        clearTimeout(timeout)
        console.warn('[IndexedDB] Delete blocked timeout, resolving anyway')
        dbInstance = null
        dbInitPromise = null
        resolve()
      }, 1000)
...
export async function deleteDB(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Timeout to prevent hanging indefinitely
    const timeout = setTimeout(() => {
      console.warn(`[IndexedDB] Delete database timeout, resolving anyway`)
      resolve()
    }, 5000)
